...
class BECLogger:
    ...
    def _update_sinks(self):
        ...
        self.logger.add(sys.stderr, level=self._log_level, format=self.format, enqueue=True)
        if self.service_name:
            self.logger.add(
                f"{self.service_name}.log", level=self._log_level, format=self.format, enqueue=True
            )
