...
Note, you need to explicitly install all packages in editable mode (with `-e` flag), to allow changes to the code base.

```bash
pip install -e './bec_lib/.[dev]'
pip install -e '../ophyd_devices/.[dev]'
pip install -e './scan_server/.[dev]' -e './scan_bundler/.[dev]' -e './data_processing/.[dev]' -e './file_writer/.[dev]' -e './device_server/.[dev]' -e './scihub/.[dev]' -e './bec_client/.[dev]' -e './bec_server/.[dev]'
```
```{note}
