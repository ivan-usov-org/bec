...
class DeviceManagerDS(DeviceManagerBase):
    ...
    def _obj_flyer_callback(self, *_args, **kwargs):
        ...
        self.producer.send(MessageEndpoints.device_read(obj.root.name), bundle.dumps())
        msg = BECMessage.DeviceStatusMessage(device=obj.root.name, status=max_points, metadata=metadata)
